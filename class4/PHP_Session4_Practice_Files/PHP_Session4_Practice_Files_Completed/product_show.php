<?php 
	...
	$sql='SELECT product_id, company, type, roast, description  FROM product ORDER BY company DESC'; 
				...
				while($recording=mysqli_fetch_array($result)){
					...
					$sql_company='SELECT name FROM company WHERE company_id="$company"';					/*Tell me if there is an error. Result and sql are named differently to avoid confusion with the sql statement and error above*/
